...
io.on("connection", (socket) => {
  ...
  socket.on("new_user", (msg) => {
    ...
    var matchingSessions = findMatchingSession(msg["game_id"], listOfPlayers);
    //console.log(matchingSessions);
    printPlayers(listOfPlayers);

    if (matchingSessions.length == 2) {
      console.log("matching sessions");
      listOfPlayers[matchingSessions[0]]["socket"].emit(
        "found_match",
        listOfPlayers[matchingSessions[1]]["username"]
      );
      listOfPlayers[matchingSessions[1]]["socket"].emit(
        "found_match",
        listOfPlayers[matchingSessions[0]]["username"]
      );
    }
  });
  ...
  socket.on("choice", (choice) => {
    ...
    var matchingSessions = findMatchingSession(
      listOfPlayers[id_]["game_id"],
      listOfPlayers
    );
    ...
    if (
      listOfPlayers[id_]["choices"].length >
      listOfPlayers[id_other]["choices"].length
    )
      return;
    ...
    if (
      ...
      var choice_other =
        listOfPlayers[id_other]["choices"][
          listOfPlayers[id_other]["choices"].length - 1
        ];
  ...
  socket.on("disconnect", () => {
    var idToRemove = findSocket(socket, listOfPlayers);
    if (idToRemove == -1) return;

    var matchingSessions = findMatchingSession(
      listOfPlayers[idToRemove]["game_id"],
      listOfPlayers
    );
